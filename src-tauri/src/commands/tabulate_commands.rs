...
use crate::models::tabulate::{
    TabulateMaterializeRequest, TabulateSessionRequest, TabulateSessionStatus,
...
pub fn query_tabulate_window(
    ...
    let service = state
        ...
        .map_err(|error| AppError::Database(error.to_string()))?
        .clone();
    service.query_window(&request)
}

#[tauri::command]
pub fn cancel_tabulate_request(
    state: State<'_, AppState>,
    request_id: String,
) -> Result<(), AppError> {
    let service = state
...
mod tests {
    use crate::models::tabulate::{
        StatisticKind, TabulateMaterializeRequest,
