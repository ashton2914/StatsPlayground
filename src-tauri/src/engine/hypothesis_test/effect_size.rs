...
pub fn hedges_g_pooled(groups: &IndependentGroups) -> Result<EffectEstimate, AppError> {
    ...
    let pooled_variance = (((left.values.len() - 1) as f64 * left_variance)
        + ((right.values.len() - 1) as f64 * right_variance))
        / degrees_of_freedom;
    effect(
...
pub fn paired_d_z(differences: &PairedDifferences) -> Result<EffectEstimate, AppError> {
    ...
    if denominator <= 0.0 {
        return Err(AppError::Stats(
            "paired difference standard deviation is zero".into(),
        ));
    }
    Ok(EffectEstimate {
...
fn two_groups(
    ...
    (
        &crate::engine::hypothesis_test::normalize::ConditionValues,
        &crate::engine::hypothesis_test::normalize::ConditionValues,
    ),
    AppError,
> {
    if groups.groups.len() != 2 {
        return Err(AppError::Stats(
            "effect size requires exactly two groups".into(),
        ));
    }
...
fn effect(
    ...
    if denominator <= 0.0 || !denominator.is_finite() {
        return Err(AppError::Stats(
            "effect size denominator is not estimable".into(),
        ));
    }
...
fn hedges_correction(degrees_of_freedom: f64) -> Result<f64, AppError> {
    if degrees_of_freedom <= 1.0 || !degrees_of_freedom.is_finite() {
        return Err(AppError::Stats(
            "Hedges correction requires more than one degree of freedom".into(),
        ));
    }
