...
pub fn student_two_sample_t(
    ...
    let pooled_variance = (((left.len() - 1) as f64 * left_variance)
        + ((right.len() - 1) as f64 * right_variance))
        / degrees_of_freedom;
    let standard_error =
        (pooled_variance * (1.0 / left.len() as f64 + 1.0 / right.len() as f64)).sqrt();
    finish_t(
...
pub fn welch_two_sample_t(
    ...
    if denominator <= 0.0 {
        return Err(AppError::Stats(
            "Welch degrees of freedom are not estimable".into(),
        ));
    }
...
fn finish_t(
    ...
    {
        return Err(AppError::Stats(
            "t statistic is not estimable from zero or invalid variance".into(),
        ));
    }
    if !confidence_level.is_finite() || confidence_level <= 0.0 || confidence_level >= 1.0 {
        return Err(AppError::InvalidParam(
            "confidence level must be inside (0, 1)".into(),
        ));
    }
    ...
    let (p_value, lower_probability, upper_probability) = match alternative {
        ...
        HypothesisTestAlternative::Less => {
            (distribution.cdf(statistic), 1.0 - confidence_level, 1.0)
        }
        HypothesisTestAlternative::Greater => (distribution.sf(statistic), 0.0, confidence_level),
    };
...
mod tests {
    ...
    fn paired_t_matches_reference_and_is_affine_invariant() {
        ...
        let result = paired_t(&pairs, HypothesisTestAlternative::TwoSided, 0.95).expect("paired t");
    ...
    fn student_and_welch_use_distinct_variance_and_df_models() {
        ...
        let student =
            student_two_sample_t(&left, &right, HypothesisTestAlternative::TwoSided, 0.95)
                .expect("Student t");
        let welch = welch_two_sample_t(&left, &right, HypothesisTestAlternative::TwoSided, 0.95)
            .expect("Welch t");
    ...
    fn one_sided_alternatives_use_directional_tails_and_infinite_interval_bound() {
        ...
        let less = welch_two_sample_t(&left, &right, HypothesisTestAlternative::Less, 0.95)
            .expect("less Welch t");
        let greater = welch_two_sample_t(&left, &right, HypothesisTestAlternative::Greater, 0.95)
