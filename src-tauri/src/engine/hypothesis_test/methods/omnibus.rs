...
pub fn one_way_anova(groups: &IndependentGroups) -> Result<OmnibusResult, AppError> {
    ...
    if mean_square_within <= 0.0 {
        return Err(AppError::Stats(
            "ANOVA residual variance is not estimable".into(),
        ));
    }
    let statistic = (between / df1) / mean_square_within;
    let distribution =
        FisherSnedecor::new(df1, df2).map_err(|error| AppError::Stats(error.to_string()))?;
    let effect_size =
        (between - df1 * mean_square_within) / (between + within + mean_square_within);
    Ok(OmnibusResult {
        statistic,
...
pub fn welch_anova(groups: &IndependentGroups) -> Result<OmnibusResult, AppError> {
    ...
    if group_moments
        .iter()
        .any(|(_, _, variance)| *variance <= 0.0)
    {
        return Err(AppError::Stats(
            "Welch ANOVA requires positive variance in every group".into(),
        ));
    }
    let weights = group_moments
        .iter()
        .map(|(count, _, variance)| count / variance)
        .collect::<Vec<_>>();
    let weight_sum = weights.iter().sum::<f64>();
    let weighted_mean = group_moments
        .iter()
        .zip(&weights)
        .map(|((_, mean, _), weight)| mean * weight)
        .sum::<f64>()
        / weight_sum;
    let groups_count = group_moments.len() as f64;
    let df1 = groups_count - 1.0;
    let weighted_between = group_moments
        .iter()
        .zip(&weights)
        .map(|((_, mean, _), weight)| weight * (mean - weighted_mean).powi(2))
        .sum::<f64>()
        / df1;
    let correction_sum = group_moments
        .iter()
        .zip(&weights)
        .map(|((count, _, _), weight)| (1.0 - weight / weight_sum).powi(2) / (count - 1.0))
        .sum::<f64>();
    let correction =
        1.0 + 2.0 * (groups_count - 2.0) * correction_sum / (groups_count.powi(2) - 1.0);
    let statistic = weighted_between / correction;
    let df2 = (groups_count.powi(2) - 1.0) / (3.0 * correction_sum);
    let distribution =
        FisherSnedecor::new(df1, df2).map_err(|error| AppError::Stats(error.to_string()))?;
    let effect_size = ((statistic - 1.0) * df1) / (statistic * df1 + df2 + 1.0);
    Ok(OmnibusResult {
        statistic,
        numerator_degrees_of_freedom: df1,
        denominator_degrees_of_freedom: Some(df2),
...
pub fn kruskal_wallis(groups: &IndependentGroups) -> Result<OmnibusResult, AppError> {
    ...
    let tie_correction = 1.0
        - ties.iter().map(|size| size.pow(3) - size).sum::<usize>() as f64
            / (count.pow(3) - count) as f64;
    if tie_correction <= 0.0 {
        return Err(AppError::Stats(
            "Kruskal-Wallis is undefined when all observations tie".into(),
        ));
    }
...
fn validate_groups(groups: &IndependentGroups, allow_singletons: bool) -> Result<(), AppError> {
    if groups.groups.len() < 2 {
        return Err(AppError::Stats(
            "omnibus test requires at least two groups".into(),
        ));
    }
    ...
    if groups.groups.iter().any(|group| {
        ...
        return Err(AppError::Stats(
            "omnibus groups have insufficient finite observations".into(),
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    use crate::engine::hypothesis_test::methods::parametric::{
        student_two_sample_t, welch_two_sample_t,
    };
    ...
    fn two_group_anova_matches_corresponding_t_squared() {
        ...
        assert!(
            (welch_anova.denominator_degrees_of_freedom.unwrap() - welch.degrees_of_freedom).abs()
                < 1e-10
        );
    }
    ...
    fn fixture() -> IndependentGroups {
        IndependentGroups {
            groups: vec![
                ...
                ConditionValues {
                    condition: "B".into(),
                    values: vec![20.0, 22.0, 24.0, 26.0, 28.0],
                },
            ],
        }
    }
