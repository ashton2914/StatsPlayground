...
assertSourceIncludes(workspaceSource, "analyses: analysisItems", "Project save payload must include analyses");
assertSourceIncludes(workspaceSource, "loadAnalyses((result.analyses ?? [])", "Project open must hydrate analyses from project data");
...
assertSourceIncludes(workspaceSource, "history.newAnalysis", "Creation must record Analysis history");
...
assertSourceIncludes(workspaceSource, "setActiveAnalysisId(null)", "Opening any other document or resetting the workspace must clear the active Analysis");
assertSourceIncludes(workspaceSource, "setActiveAnalysisId(id)", "Selecting or creating an Analysis must activate it");
...
assertSourceIncludes(workspaceSource, "activeAnalysis?.source.datasetId === id", "Source-table deletion must recognize an active Analysis whose source dataset was deleted");
